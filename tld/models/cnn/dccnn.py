...
def build_dccnn_model(max_seq_length: int):
    ...
    return Model(inputs=[text_in], outputs=[conv_concat], name='DC-CNN_Model')


def build_branch_a(text_in: Input, max_seq_length: int):
    conv_a = Conv2D(
        filters=100,
        kernel_size=(1, 20),
        strides=(1, 1),
    ...
    conv_a_rs = Reshape((max_seq_length, 100, 1))(conv_a)
...
def build_branch_b(text_in: Input, max_seq_length: int):
    conv_b = Conv2D(
        filters=100,
        kernel_size=(2, 20),
        activation='relu',
    ...
    conv_b_rs = Reshape((max_seq_length - 1, 100, 1))(conv_b)
...
def build_branch_c(text_in, max_seq_length: int):
    ...
    conv_c_rs = Reshape((max_seq_length - 2, 100, 1))(conv_c)
